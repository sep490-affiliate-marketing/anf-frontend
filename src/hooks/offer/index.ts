...
import { IGetOfferResponse, IOffer, IUpdateOfferErrorResponse, IUpdateOfferSuccessResponse } from "@/types/offer.type"
...
export const useUpdateOfferForm = (id: number, campaignId: number) => {
  ...
  const { mutateAsync: updateOfferMutation, isPending } = useMutation({
    ...
    mutationFn: async (
      ...
      IUpdateOfferSuccessResponse | IUpdateOfferErrorResponse
    > => {
      try {
    ...
    onSuccess: async (resData) => {
      if (resData.isSuccess === true) {
        ...
        queryClient.invalidateQueries({
          queryKey: offerQueryKeys.advertiser.update(
            id.toString()
          ),
        })
        queryClient.invalidateQueries({
          queryKey: offerQueryKeys.advertiser.details(
            id.toString()
          ),
        })
        form.reset()

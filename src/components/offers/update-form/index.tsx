...
const EditOfferForm = ({ offerId }: Props) => {
  ...
  const { data: offerResponse, isLoading, isFetching,} = useGetOfferById(offerId)
  const { form, isPending, onUpdateOffer } = useUpdateOfferForm(Number(offerId), offerResponse?.data?.campaignId || 0)
  const { data: CampaignResponse} = useGetCampaignById(offerResponse?.data?.campaignId.toString() || "")
  ...
  useEffect(() => {
      ...
      setValue('orderReturnTime', offerResponse?.data?.orderReturnTime.toString().split(" ")[0] || '');
      setValue('startDate', offerResponse?.data?.startDate || '');
      setValue('endDate', offerResponse?.data?.endDate || '');
      setValue('commissionRate', offerResponse?.data?.commissionRate.toString() || '');
  ...
  const getDateRange = (): DateRange | undefined => {
    return {
      from: offerResponse?.data?.startDate ? new Date(offerResponse?.data?.startDate) : undefined,
      to: offerResponse?.data?.endDate ? new Date(offerResponse?.data?.endDate) : undefined,
    }
  }
        ...
        <div className="border-b border-border pb-6">
          ...
          <div className="grid grid-cols-1 gap-6 md:grid-cols-2">
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  <FormLabel className="text-lg font-semibold">Pricing Model</FormLabel>
                  <Popover
            ...
            <FormField
              control={form.control}
              name={form.watch(`pricingModel`) === "CPS"
                ? "bid"
                : "commissionRate"}
            ...
            <div className="space-y-2 md:col-span-1">
              <FormLabel className="text-lg font-semibold">Offer Date Range</FormLabel>
              <DatePickerWithRange
                className="w-full"
                ...
                onChange={(dates: {
                  ...
                  if (dates.startDate) {
                    ...
                    setDateRange({
                      ...
                      to: dates.endDate ? new Date(dates.endDate).toISOString() : '',
                    })
          ...
          <div className="flex items-center justify-between pt-4">
              <Button
                ...
                variant="outline"
                onClick={() => {
                  router.push(`/advertiser/campaigns/${offerResponse?.data?.campaignId}/offers/${offerId}`)
                }}
                className="h-11"
              >
                Back
